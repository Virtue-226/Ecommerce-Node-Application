...
import { ProductSchema, UpdateProductSchema } from "../schema/users";
...
//   const productExists = await prismaCilient.product.findFirst({
//     where: {
//       name: req.body.name,
//     },
//   });

  const product = await prismaCilient.product.create({
    data: {
      ...req.body,
...
export const getProducts = async (req: Request, res: Response) => {
  ...
  const products = await prismaCilient.product.findMany({
    where: {
      name: {
...
export const getProductById = async (req: Request, res: Response) => {
  try {
    const { id } = req.params;
    const product = await prismaCilient.product.findFirstOrThrow({
      where: {
        id: Number(id),
      },
    });
...
export const updateProduct = async (req: Request, res: Response) => {
  ...
  try {
    const updatedProduct = await prismaCilient.product.update({
      where: {
        id: Number(id),
      },
      data: product,
...
export const deleteProduct = async (req: Request, res: Response) => {
  const { id } = req.params;
  await prismaCilient.product.delete({
    where: {
      id: Number(id),
    },
  });
...
export const aggregateTotalPrice = async (req: Request, res: Response) => {
  const totalPrice = await prismaCilient.product.aggregate({
    where: {
      price: {
        gt: 0,
      },
...
export const groupByPrice = async (req: Request, res: Response) => {
  const products = await prismaCilient.product.groupBy({
    by: ["price"],
    where: {
      price: {
        // not: 90.78,
